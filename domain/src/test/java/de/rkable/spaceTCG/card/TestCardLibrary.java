...
public class TestCardLibrary {
	...
	private final static int NUM_TIER1_CARDS = 2;
	private final static int NUM_TIER2_CARDS = 2;
	private final static int NUM_TIER3_CARDS = 2;

	@Test
	public void getTier1Cards_givesAllTier1Cards() {
		CardLibrary cardLibrary = new CardLibrary();
		List<CardFactory> commonCards = cardLibrary.getTier1Cards();
		assertEquals(NUM_TIER1_CARDS, commonCards.size());
	}
	@Test
	public void getTier2Cards_givesAllTier2Cards() {
		CardLibrary cardLibrary = new CardLibrary();
		List<CardFactory> commonCards = cardLibrary.getTier2Cards();
		assertEquals(NUM_TIER2_CARDS, commonCards.size());
	}
	@Test
	public void getTier3Cards_givesAllTier3Cards() {
		CardLibrary cardLibrary = new CardLibrary();
		List<CardFactory> commonCards = cardLibrary.getTier3Cards();
